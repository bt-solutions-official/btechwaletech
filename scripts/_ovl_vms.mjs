...
for (const t of targets) { const mine = sh.get(t); const hits = new Map(); for (const s of mine) for (const o of index.get(s)) if (o !== t) hits.set(o, (hits.get(o) ?? 0) + 1); console.log(t, [...hits].sort((a,b)=>b[1]-a[1]).slice(0,4).map(([o,h])=>o+" "+(100*h/mine.size).toFixed(1)+"%").join(", ")); }
process.exit(0);
let failed = 0;
for (const { file, location, c, error } of all) {
  ...
  if (error) problems.push(`does not compile: ${error}`);
    ...
    for (const [other, hit] of hits) {
      ...
      if (ratio > MAX_OVERLAP) problems.push(`${(ratio * 100).toFixed(1)}% of phrases copied from ${other}, max ${MAX_OVERLAP * 100}%`);
    }
    if (!problems.length) console.log(`PASS ${c.slug}: ~${total} words`);
  }
  if (problems.length) {
    failed++;
