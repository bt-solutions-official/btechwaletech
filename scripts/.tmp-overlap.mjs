...
for (const { file, location, c, error } of all) {
  ...
  if (error) problems.push(`does not compile: ${error}`);
    ...
    { const top=[...hits].sort((a,b)=>b[1]-a[1]).slice(0,3).map(([o,h])=>o+" "+(h/mine.size*100).toFixed(1)+"%").join(", "); console.log(`${problems.length?"FAIL":"PASS"} ${c.slug}: ~${total} words; top overlap: ${top}`); }
  }
  if (problems.length) {
    failed++;
